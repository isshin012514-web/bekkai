...
type TabKey = 'all' | 'input' | 'output' | 'review'
...
type RecordItem =
  | { kind: 'input'; data: Input; at: string }
  | { kind: 'output'; data: Output; at: string }
  | { kind: 'review'; data: Output; at: string }
...
export function RecordsSection({ inputs, outputs, onSelectOutput }: RecordsSectionProps) {
  ...
  const reviewCount = outputs.filter((o) => o.peer_score != null).length

  const items: RecordItem[] = [
    ...inputs.map((i): RecordItem => ({ kind: 'input', data: i, at: i.created_at })),
    ...outputs.map((o): RecordItem => ({ kind: 'output', data: o, at: o.created_at })),
    ...outputs
      .filter((o) => o.peer_score != null && o.peer_scored_at)
      .map((o): RecordItem => ({ kind: 'review', data: o, at: o.peer_scored_at! })),
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    {output.self_score_detail && (
                      <span className="text-[10px] text-text-tertiary">
                        独{output.self_score_detail.originality} 実{output.self_score_detail.practicality} 完{output.self_score_detail.completeness}
                      </span>
                    )}
                  </div>
                </div>
